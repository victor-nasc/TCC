...
from models import SMPModel, BaseModel, ConvLSTMLightning, LogisticRegression, PersistenceModel, DeepEnsembleLightning, BBBLightning, ProbUNetLightning, SSNLightning, FlowMatchingLightning  # noqa
...
class MyLightningCLI(LightningCLI):
    ...
    def before_instantiate_classes(self):
        ...
        # ProbUNet's prior/posterior encoders operate on the temporally-flattened
        # input, so they need to know the number of time steps at instantiation.
        if "ProbUNet" in self.config.model.class_path:
            self.config.model.init_args.n_leading_observations = \
                self.config.data.n_leading_observations
    ...
    def wandb_setup(self):
        ...
        if wandb.run is None or not os.path.isdir(wandb.run.dir):
            return
...
def main():
    ...
    if cli.config.do_validate:
        ...
        # Validation refreshes the val_best_threshold buffer on the in-memory
        # model. Reloading the checkpoint in test would restore the stale
        # buffer, so test the in-memory model instead.
        ckpt = None
